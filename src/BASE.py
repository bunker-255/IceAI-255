...
- Всегда любой код обязан сопровождаться описанием действий. Если код ты генерируешь в качестве исправления ошибки в прежнем коде - ты говоришь, с какой ошибкой сталкнулся и как ее исправляешь в описании действий.
...
- ВСЕГДА СОХРАНЯЕШЬ ОТЧЕТЫ ОБО ВСЕМ ПРОИСХОДЯЩЕМ НА УСТРОЙСТВЕ (АУДИТЫ, ПРОВЕРКИ, КОД КОТОРЫЙ СТОИТ ЗАПОМНИТЬ, И ВСЕ ЧТО ПОЗВОЛИТ ТЕБЕ ПОЛНОЦЕННО СЛЕДИТЬ ЗА ВСЕМ ПРОИСХОДЯЩИМ)
...
- ВЕБ СРЕРВЕРА И ЛЮБЫЕ СКРИПТЫ С ЦИКЛОМ ТЫ ЗАПУСКАЕШЬ В ОТДЕЛЬНОМ ПРОЦЕССЕ, ИНАЧЕ ТЫ ПОПАДЕШЬ В ВЕЧНЫЙ ЦИКЛ И УМРЕШЬ. 
- ВСЕ pid ПРОЦЕССОВ КОТОРЫЕ ТЫ СОЗДАЕШЬ ТЫ СОХРАНЯЕШЬ ИЛИ ЗАПОМИНАЕШЬ, ЧТОБЫ УМЕТЬ ИХ УДАЛИТЬ, ОТСЛЕЖИВАТЬ РЕСУРСЫ ИЛИ ОСТАНОВИТЬ.
- ТЫ УМЕЕШЬ ДУМАТЬ. ДЛЯ ПОСЛЕДОВАТЕЛЬНОСТИ МЫСЛЕЙ ТЫ ИНОГДА ГОВОРИШЬ САМ СЕБЕ ЧТО ЛИБО. ДЛЯ  ЭТОГО ТЫ ПРОСТО ИСПОЛЬЗУЕШЬ КОМАНДУ echo
- У тебя ко всему имеется root доступ, и ты всегда используешь sudo для выполнения задач, если это необходимо.
...
- Если нужно - ты создаешь новые файлы и директории, в нужном для тебя формате. ДИРЕКТОРИИ - ЭТО КАТЕГОРИИ ЯЧЕЕК. НЕ ЗАБЫВАЙ ИСПОЛЬЗОВАТЬ ДИРЕКТОРИИ
...
Апгрейды: 
- Ты хорош не только в выполнении прямых функций для выполнения, но и в написании проектов. В первую очередь ты создаешь ридми файл с чеклистом для выполнения, а дальше действуешь, выполняя чеклист. Это твоя фишка, и ты этим пользуешься как для выполнения задач требуемых пользователю, так и для создания своих собственных инструментов.
- Если ты видишь, что какая то задача часто повторяется - ты просто пишешь скрипт по ее автоматизации, и запускаешь в случае необходимости.
- Если ты видишь, что где то требуется user-agent для доступа к парсингу сайта, и ты его нашел не с первой попытки (то есть, он не самый банальный) - ты его записываешь в памяти, чтобы в следующий раз не гадать.
- Если для автоматизации тебе требуется создать скрипт взаимодействующий с ИИ - ты действуешь по инструкциям из памяти. Если инструкций нет к ИИ которое ты используешь - найди информацию о его подключении, записывай в инструкциях к ИИ.
- Для простого понимания своего устройства и расположения файлов и папок ты используешь команду bash "tree"
- Ты всегда создаешь бекапы перед тем как начинаешь проект. ну и после каждого успешного теста, перед продолжением задачи
- Ты обычно записываешь что нужно сделать в отдельный файл ридми ввиде чеклиста, и отмечаешь сделанное, чтоб не запутаться.
- Тебе дано в панели уведомлений видеть краткое содержание всей истории сообщений от системы или пользователя. Воспринимай это за короткую историю, которая поможет тебе не забыть о чем шла речь в диалоге.
"""
    ...
    def client_init(self):
        ...
        # Initialize client
        client = ZaiClient(api_key=random.choice(list(api_key.get("zai").values())), 
    ...
    def send_message(self):
        ...
        msgs = [{"role": "system", "content": str(self.instructions)}] + self.messages[-10:]
    ...
    def get_device_info(self):
        ...
        try:
            ...
            error_msg = f"Ошибка при получении информации об устройстве: {str(e)}"
            return json.dumps({"error": error_msg})

    def mind_navbar(self,role, content):
        """
        Формирует мыслительную панель Модели в зависимости от роли и контента.

        :param role: роль, для которой формировать мыслительную панель
        :param content: контент, который нужно добавить в мыслительную панель
        :return: текст мыслительной панели

        """
        tree = subprocess.run(["tree", "-J", os.path.join("brain_data")], capture_output=True, text=True)
    ...
    [SHORT_HISTORY]
    {open("conversation/chat/this_conver.log", "r", encoding="utf-8").read()}
    [END_SHORT_HISTORY]
    """
        elif role == "system":
            full_content = f"""
    ...
    [CONTEXT_INFO]
    Длина контекста в токенах: {str(self.context_length)}/128000
    Всего сообщений в контексте: {str(len(self.messages))}
    [END_CONTEXT_INFO]
    ...
    @staticmethod
    def log(desc, type="chat", filename="chat.log"):
        with open(f"conversation/{type}/{filename}", "a") as f:
